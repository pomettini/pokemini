...
#ifdef PERFORMANCE
static inline __attribute__((always_inline)) uint8_t MinxCPU_CF_LocalRead(uint32_t addr)
{
	if (addr >= 0x2100) {
...
POKEMINI_HOT int MinxCPU_ExecCF(void)
{
	uint8_t I8A;
	uint16_t I16;
	...
	MinxCPU_OpcodeDiag[MINXCPU_OPDIAG_CF][MinxCPU.IR]++;
#endif
